  ...
  useEffect(() => {
    ...
    const fetchRate = async () => {
      try {
        ...
        if (!cancelled) {
          if (!error && data) {
            setRetefuenteCompraRate(data.retefuente_compra_rate || 0);
          }
          setRateLoaded(true);
        }
      } catch (err) {
  ...
  const summaryByMonth = useMemo(() => {
    ...
    for (const inv of invoices) {
      try {
        ...
        const s = map.get(key)!;
        if (inv.type === 'venta') {
          s.ventasBase += inv.subtotal_base || 0;
          s.ventasIva += inv.iva_amount || 0;
          s.ventasReteica += inv.reteica_amount || 0;
          s.ventasAutoretefuente += inv.autoretefuente_amount || 0;
        } else {
          s.comprasBase += inv.subtotal_base || 0;
          s.comprasIva += inv.iva_amount || 0;
          s.comprasRetefuente += Math.round((inv.subtotal_base || 0) * retefuenteCompraRate);
        }
      } catch {
        // Skip invoices with problematic data
        continue;
      }
    }
    return Array.from(map.values()).sort((a, b) => a.month.localeCompare(b.month));
  }, [invoices, retefuenteCompraRate, rateLoaded]);
