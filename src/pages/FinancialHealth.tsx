...
function getVariableAlert(key: string, score: number): string | null {
  // Umbrales escalados al nuevo techo /25 (cada variable pesa 25, ~88% del max gatilla alerta).
  if (key === 'conciliacion' && score < 22) return 'Movimientos sin soporte detectados';
  if (key === 'facturacion' && score < 22) return 'Ingresos sin factura detectados';
  if (key === 'impuestos' && score < 20) return 'Descuadre alto entre Siigo y físico';
  if (key === 'cartera' && score < 22) return 'Facturación sin cobrar o anticipos pendientes';
  return null;
}
