-- Fix al pronóstico de flujo de caja (forecast_cashflow):
--
-- BUG: el modelo extrapolaba el GASTO operativo recurrente 90 días hacia
-- adelante pero NO tenía el análogo de INGRESOS recurrentes. Un usuario sin
-- facturas vivas ni promesas registradas veía "Entradas esperadas = $0" y una
-- proyección lineal al abismo (-175M/mes acumulativo) aunque reciba dinero
-- todas las semanas. Asimetría estructural, no un problema de datos.
--
-- Cambios:
--   1. v_recurring_inflow_daily: promedio diario de ingresos de los últimos
--      90 días sin factura vinculada (espejo exacto del gasto recurrente).
--      Se suma a los inflows de cada día proyectado.
--   2. Ambos recurrentes (ingreso y gasto) excluyen movimientos cuya
--      movement_nature no es operativa (traspaso/préstamo/aporte/devolución).
--      Antes un "PAGO TARJETA" marcado traspaso inflaba el gasto recurrente
--      y duplicaba las compras de la tarjeta que ya entran como gasto.
--   3. inflow_sources reporta 'recurring_estimated' para transparencia en la UI.
--
-- Mismo signature y return type → CREATE OR REPLACE seguro.
...
DECLARE
  ...
  v_recurring_inflow_daily numeric := 0;
...
BEGIN
  ...
  -- Flujo operativo recurrente: promedio diario de últimos 90 días excluyendo
  -- (a) transacciones vinculadas a facturas (ya contadas aparte) y
  -- (b) movimientos no operativos (traspasos entre cuentas propias, préstamos,
  --     aportes, devoluciones) que no son ingreso/gasto real del negocio.
  SELECT
    COALESCE(SUM(ABS(amount)) FILTER (WHERE COALESCE(amount, 0) < 0) / GREATEST(90, 1), 0),
    COALESCE(SUM(amount) FILTER (WHERE COALESCE(amount, 0) > 0) / GREATEST(90, 1), 0)
  INTO v_recurring_outflow_daily, v_recurring_inflow_daily
  FROM public.transactions
  WHERE user_id = v_user
    AND deleted_at IS NULL
    AND date >= v_today - 90
    ...
    AND COALESCE(movement_nature, 'operativo') = 'operativo';
  ...
  joined AS (
    SELECT
      ...
      COALESCE(ep_per_day.total, 0)
        + COALESCE(inv_venta_per_day.total_weighted, 0)
        + v_recurring_inflow_daily AS inflows,
      ...
      jsonb_build_object(
        ...
        'recurring_estimated', v_recurring_inflow_daily,
...
COMMENT ON FUNCTION public.forecast_cashflow(uuid, integer) IS
  'Predicción día por día (hasta 180 días): promesas + facturas vivas con scoring IA + créditos + flujo operativo recurrente (ingresos Y gastos, solo naturaleza operativa).';
